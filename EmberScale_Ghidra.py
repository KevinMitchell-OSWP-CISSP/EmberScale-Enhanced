...
# @author Kevin Mitchell aka kalikev
...
# -------------------------
_API_KEY_CACHE = None  # global cache across actions
...
def _read_key_from_files():
    """Try ~/.anthropic/apikey then ~/.config/anthropic/apikey."""
    try:
        from java.lang import System as JSystem
        from java.io import File
        home = _to_text(os.path.expanduser("~"), "")
        candidates = [
            os.path.join(home, ".anthropic", "apikey"),
            os.path.join(home, ".config", "anthropic", "apikey"),
        ]
        for p in candidates:
            try:
                if os.path.isfile(p):
...
def _save_key_to_prefs(k):
    try:
        prefs = Preferences
        prefs.setProperty("ANTHROPIC_API_KEY", k)
        prefs.store()  # persist to disk
        return True
    except Exception:
        return False
...
def _ensure_api_key(key_candidate=None):
    ...
    # 1) explicit arg
    k = _to_text(key_candidate, "").strip()
    # 2) env
    if not k:
        k = _to_text(os.environ.get("ANTHROPIC_API_KEY"), "").strip()
    if not k:
        k = _to_text(os.environ.get("CLAUDE_API_KEY"), "").strip()
    # 3) Java system properties (if launching via wrapper)
    if not k:
        try:
            from java.lang import System as JSystem
            k = _to_text(JSystem.getProperty("ANTHROPIC_API_KEY"), "").strip()
            if not k:
                k = _to_text(JSystem.getProperty("CLAUDE_API_KEY"), "").strip()
        except Exception:
            pass
    # 4) Ghidra Preferences
    if not k:
        k = _read_key_from_prefs()
    # 5) Files
    if not k:
        k = _read_key_from_files()

    # 6) Prompt once if still missing
    if not k:
        try:
            entered = askString("Anthropic API Key", "Enter your Anthropic API key (will be saved to Ghidra Preferences):", "")
            if entered and entered.strip():
                k = entered.strip()
                # Save so you’re not asked again
                if not _save_key_to_prefs(k):
                    log_warn("Could not persist API key to Ghidra Preferences; it will only be cached for this session.")
        except Exception:
            log_err("Anthropic API key missing and prompt unavailable. Set ANTHROPIC_API_KEY in env or Preferences.")
            raise SystemExit(1)

    _API_KEY_CACHE = k  # cache for this session
    return k
# -------------------------
...
def main():
    # Track usage
    track_usage_for_operation("Ghidra_Agent_Analysis")
    run_agent_mode()
