...
import decxy.api as decxy_api  # module import so we can patch it
from decxy.decompiler import decompile_function
from decxy.utils import (
...
def _ensure_api_key(key_candidate=None):
    # First check Ghidra Preferences
    try:
    ...
    if key:
        ...
        try:
            from ghidra.framework.preferences import Preferences
            prefs = Preferences.getUserPreferences()
            prefs.putString("ANTHROPIC_API_KEY", key.strip())
            prefs.flush()
...
# -------------------------
# Patch decxy.api with a safe wrapper (monitor-safe)
# -------------------------
_ORIG_GET = decxy_api.get_response_from_claude
...
# Patch module symbol so any code using decxy.api gets the safe version
decxy_api.get_response_from_claude = get_response_from_claude_safe
